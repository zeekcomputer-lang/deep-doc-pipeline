...
> **최종 업데이트:** 2026-06-01
> **상태:** v1.3.1 코드 완성 + AST 검증 통과 / **실제 LLM 실행 미수행** (사용자 환경 보류)
...
**규모:** 11 파일 / 2,407줄. AST 문법 검증 전 파일 통과.
...
└── src/
    ├── schemas.py         8종 Pydantic (TranslationCheckResult 포함)
    ├── state.py           GraphState — 3개 reducer + 번역 상태 5필드
    ├── context_guard.py   ★ 95KB 예산 관리 (측정/분할/교차검증)
    ├── llm.py             ★ GPT-OSS 표준 + Rate Limiter + 언어 중립 JSON guard
    ├── utils.py           Pure Python (sort/filter/compile/validate/split/extract_proper_nouns/validate_korean_structure)
    ├── nodes.py           20개 노드 + 5개 라우터 (Phase 1~5)
