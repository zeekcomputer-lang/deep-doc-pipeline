...
**규모:** 12 파일 / 2,622줄. AST 문법 검증 전 파일 통과.
...
| 버전 | 커밋 | 날짜 | 핵심 변경 |
|------|------|------|----------|
| **v1.4** | `a6677ea` | 2026-06-01 | 504 국부 감축 + max_tokens + reasoning_effort + best-of-N + 영문분리 |
...
**v1.3 변경 (EN-only + 백서 전용 + 렌더링):**
1. 영어 전용 LLM 출력 (`_EN_ENFORCE`), JSON guard는 언어 중립
2. `status_report` 모드 완전 제거
3. EN→KR 수석 에디터 렌더링 (`## 연도` / `### 월:` + 통합 서술 + 평어체)
4. 렌더링 3중 검증 (Python 고유명사 + 구조 + LLM 스팟체크)

**v1.4 변경 (504 방어 + 품질 제어 + 이중 출력):**
1. **504 국부 감축:** 504 발생 시 해당 노드만 예산 -5KB/step 적용, 성공 후 원복. user 메시지 절단 금지.
2. **`max_tokens=24,000`** (95KB 이내 응답 강제) + **`reasoning_effort="high"`** 전역
3. **영문 원본 항상 분리 저장** (`output_en.md`) + 한글 렌더링 (`output.md`)
4. **번역 검증 완화:** 고유명사 임계치 50%, 최대 30회 재시도, 실패 시 상위 3건 후보 반환
5. **타임라인 로거** (`[MM:SS] #N [tag]`) + 실행 통계 (소요시간/작업수/LLM호출)
...
└── src/
    ...
    ├── state.py           GraphState + reducer (54줄)
    ├── context_guard.py   95KB 예산 관리 (165줄)
    ├── llm.py             OpenAI SDK + Rate Limiter + 504 방어 + JSON guard (438줄)
    ├── logger.py          타임라인 로거 + 실행 통계 (78줄)
    ├── utils.py           Pure Python 결정론 로직 (265줄)
    ├── nodes.py           20개 노드 + 5개 라우터 + @retry_on_504 (1,281줄)
...
- **Python 고유명사:** `extract_proper_nouns()` → 번역 후 존재 검증 (**50% 초과** 소실 시 반려, 최대 30회 재시도)
- **구조 검증:** `validate_korean_structure()` → `## YYYY년` / `### YYYY년 X월:` 헤딩 존재·개수
- **LLM 스팟체크:** 첫 2000자 영한 쌍 비교 (사실 추가/누락/고유명사 변형 검출)
- **실패 시:** 상위 3건 후보 반환 (고유명사 유지율 순)

### 방어 5: 504 국부 감축 (v1.4)
- **발생 시:** `structured_call` → `_reduce_504()` → 예산 -5KB + 토큰 -1,280 → `Timeout504Error` raise
- **노드 재실행:** `@retry_on_504` 데코레이터가 노드 전체 재실행 → 축소된 `effective_budget()`로 분할 로직이 더 작은 청크 생성
- **원복:** 노드 완료 후 `reset_504_state()` → 다음 노드는 원래 예산(95KB/24,000) 사용
- **절대 금지:** user 메시지 절단 — `_trim_longest_user_msg` 제거됨
- **변경점 2가지만:** (1) 출력 토큰 감소 (2) 입력 분할 전략 변경 (더 많은 배치). 프롬프트/데이터 동일
...
7. **user 메시지 절단 금지** — 504 방어는 노드 재실행(분할 로직 재생성)으로만 처리. 컨텐츠 절단 금지.
8. **504 감축은 국부적** — 실패 노드만 축소, 성공 후 원복. 전역 품질 저하 방지.
...
- [x] 미사용 코드/주석 정리, 전체 영어 주석 통일
- [x] **`max_tokens=24,000`** 전역 적용 (95KB 이내 응답 강제)
- [x] **`reasoning_effort="high"`** 전역 적용
- [x] **영문 원본 항상 분리 저장** (`--output-en`, 기본 `{stem}_en.md`)
- [x] **번역 검증 완화:** 임계치 50%, 30회 재시도, 실패 시 best-of-N 상위 3건
- [x] **504 국부 감축:** `@retry_on_504` 데코레이터, `Timeout504Error`, `effective_budget()`
- [x] **user 메시지 절단 금지** — `_trim_longest_user_msg` 제거
- [x] temperature 원본 역할별 차등값 복원
- [x] GitHub push (`a6677ea`)
