...
```
JSONL → 추출·압축·전략분석 → 2개 내러티브 섹션 집필 → 조립·윤문 → 한국어 번역 → DOCX 빌더
```

**규모:** 12 Python 파일 / 3,119줄 (src 2,653 + scripts 316 + main 135 + artifacts 76). AST 전 파일 통과.
...
└── src/
    ...
    ├── schemas.py           Pydantic 스키마 7종 (52줄)
    ├── state.py             GraphState (41줄)
...
START → load_docs → [fanout] strict_extractor(×N) → chrono_sorter     Phase 1: 추출
     ...
     → assembler → polish → translate                                 Phase 5: 조립·윤문·번역
