...
- **3-page DOCX 아키텍처**: 표지(1p) + 본문 2섹션(각 1p). 고정 구조로 LLM 호출 최소화.
...
| `DocumentBlueprint` | 문서 청사진 | doc_title, doc_subtitle, section_1, section_2 |
...
init_writing → section_writer ⟲ fact_checker                      Phase 4: 집필 루프
  ...
  │
  ▼
assembler (Pure Python) → polish → translate                      Phase 5: 조립·윤문·번역
  │
  ▼
...
| 노드 | 유형 | 설명 |
|------|------|------|
| `translate` | LLM | 단일 LLM 호출 (~800단어). 충실 번역 + 고유명사 보존 (`extract_proper_nouns`). 예산 초과 시 영문 원본 유지. |
...
| 페이지 | 구성 |
|--------|------|
| **표지** | 네이비(#1B365D) 타이틀(28pt) + 악센트 라인 + 서브타이틀(14pt) + 날짜 + 조직명 |
| **본문 1** | 섹션 1 헤딩(네이비 16pt 볼드) + 마크다운 본문 렌더링 |
| **본문 2** | 섹션 2 헤딩(네이비 16pt 볼드) + 마크다운 본문 렌더링 |
...
└── src/
    ├── prompt_config.py     ★ 사용자 커스텀 프롬프트 설정 (219줄)
    ├── schemas.py           Pydantic 응답 스키마 7종 (52줄)
    ├── state.py             GraphState + reducer (41줄)
    ...
    ├── nodes.py             15개 노드 + 4개 라우터 (887줄)
    ├── graph.py             LangGraph 조립 (75줄)
    ├── artifacts.py         중간 산출물 저장 (76줄)
    └── docx_builder.py      전문 DOCX 빌더 (435줄)
```
...
| 항목 | v2.0 | v3.0 |
|------|------|------|
| 구조 | 월별 N개 섹션 | 테마 기반 고정 2섹션 |
| 기획 | draft_planner ⟲ planner_critique (루프) | strategic_analyst (단일 노드) |
| 요약 | period_summarizer (3문장) | period_digest (1~2문장 + KPI) |
| 테마 | theme_analyzer (별도 노드) | strategic_analyst에 흡수 |
| 집필 | 단일 월 데이터 | 복수 월 데이터 (evidence_periods) |
| 조립 | compiler | assembler (2섹션 고정) |
| 번역 | 섹션별 분할 + 완전성검증 + 문단분할 + 소스폴백 | 단일 호출 (~800단어) |
| 윤문 | 섹션별 분할 | 단일 호출 (~800단어) |
| 출력 | .md (한글) + .md (영문) | .md 2종 + .docx (전문) |
| LLM 호출 | ~60-160회 | ~20-30회 |
| 노드 수 | 17 + 라우터 5 | 15 + 라우터 4 |
| 코드량 | 3,080줄 | 3,119줄 (-262 순감, docx_builder +435) |
| resume | 지원 (--resume) | 임시 제거 |
